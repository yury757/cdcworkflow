 ...
 */
public class ModifyFlinkOffsetBackingStore implements OffsetBackingStore {
    private static final Logger LOG = LoggerFactory.getLogger(ModifyFlinkOffsetBackingStore.class);
    ...
    private static final String GET_SQL = "with v_data as (\n" +
            "select store_key, store_value, row_number() over (partition by store_key order by ctime desc) as rn\n" +
            "from cdc_backing_store where store_key = ?\n" +
            ")\n" +
            "select store_key, store_value from v_data where rn = 1;";
    private static final String SET_SQL = "insert into cdc_backing_store(ctime, store_key, store_value) values (current_timestamp, ?, ?);";

    public ModifyFlinkOffsetBackingStore() {
        HikariConfig config = new HikariConfig();
        config.setJdbcUrl("jdbc:postgresql://myubuntu1:5432/postgres");
        config.setUsername("postgres");
        config.setPassword("postgres");
        config.addDataSourceProperty("connectionTimeout", "1000"); // 连接超时：1秒
        config.addDataSourceProperty("idleTimeout", "60000"); // 空闲超时：60秒
        config.addDataSourceProperty("maximumPoolSize", "2"); // 最大连接数：3
        ...
        return executor.submit(
                () -> {
                    ...
                    for (ByteBuffer key : keys) {
                        // modify to pg
                        String keyStr = new String(key.array());
                        String valueStr = pgGet(keyStr);
                        if (valueStr == null) {
                            continue;
                        }
                        result.put(key, ByteBuffer.wrap(valueStr.getBytes(StandardCharsets.UTF_8)));
        ...
        return executor.submit(
                () -> {
                    for (Map.Entry<ByteBuffer, ByteBuffer> entry : values.entrySet()) {
                        // modify to pg
                        String keyStr = new String(entry.getKey().array());
                        String valueStr = new String(entry.getValue().array());
                        this.pgSet(keyStr, valueStr);
    ...
    public String pgGet(String key) {
    ...
    public void pgSet(String key, String value) {
        try (Connection connection = dataSource.getConnection()) {
            PreparedStatement statement = connection.prepareStatement(SET_SQL);
            statement.setString(1, key);
